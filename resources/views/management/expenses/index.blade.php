            ...
            >
                ...
                <div class="mb-6">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-2xl font-bold">Kelola Pengeluaran</h2>
                    </div>
                    
                    <!-- Toolbar -->
                    <div class="w-full" x-data="{ showFilters: false }">
                        <div class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 mb-2">
                            <input type="text" x-model="filters.search" @keydown.enter="fetchData(1)" placeholder="Cari pengeluaran..." class="grow border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-2 h-[42px]">
                            ...
                            <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                                ...
                                <div x-show="open" x-cloak class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-20 border border-gray-200">
                                    ...
                                    <div x-show="!['Pending', 'In Process', 'Payment Issue'].includes(selectionCommonStatus)" class="px-4 py-2 text-sm text-gray-400 italic">
                                        <span x-show="selectedItems.length === 0">Tidak ada item terpilih</span>
                                        <span x-show="selectedItems.length > 0 && selectionCommonStatus === 'mixed'">Pilihan status campuran</span>
                                        <span x-show="selectedItems.length > 0 && selectionCommonStatus !== 'mixed' && !['Pending', 'In Process', 'Payment Issue'].includes(selectionCommonStatus)">Tidak ada aksi tersedia</span>
                                    </div>
                                    @endcan
                                </div>
                            </div>
                            ...
                            <div class="w-full grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-md shadow-inner mb-6 mt-4 border border-gray-200">
                                <div>
                                    ...
                                    <select x-model="filters.type" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-2">
                                        ...
                                        <option value="reimbursement">Reimbursement (Penggantian)</option>
                                ...
                                <div>
                                    ...
                                    <select x-model="filters.status" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-2">
                                        ...
                                        <option value="Pending Confirmation">Menunggu Konfirmasi</option>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <template x-for="expense in items" :key="expense.id">
                                <tr :class="{'bg-blue-50': selectedItems.includes(expense.id)}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                                            ...
                                            x-text="statusLabel(expense.status)">
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        ...
                                        <template x-if="expense.transaction && (expense.transaction.payment_issue_proofs || expense.transaction.paymentIssueProofs) && (expense.transaction.payment_issue_proofs || expense.transaction.paymentIssueProofs).length > 0">
                                            <div>
                                                ...
                                                <template x-if="expense.proof_file">
                                                     <div class="mt-1">
                                                        <button type="button" @click="openReceiptModal(expense)" class="text-xs text-gray-500 hover:text-gray-700 hover:underline">Struk Asli</button>
                                                     </div>
                                                </template>
                                            </div>
                                        </template>
                                        ...
                                        <template x-if="!expense.transaction || !(expense.transaction.payment_issue_proofs || expense.transaction.paymentIssueProofs) || (expense.transaction.payment_issue_proofs || expense.transaction.paymentIssueProofs).length === 0">
                                            <div>
                                                ...
                                                <template x-if="expense.proof_file">
                                                    <button type="button" @click="openReceiptModal(expense)" class="text-blue-600 hover:text-blue-900 hover:underline">Lihat Bukti</button>
                                                </template>
                                                <template x-if="!expense.proof_file">
                             ...
                             <tr x-show="items.length === 0 && !loading">
                                <td colspan="7" class="px-6 py-4 text-center text-gray-500">Tidak ada pengeluaran ditemukan.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                                        ...
                                        <div class="sm:flex sm:items-start">
                                            <div class="mt-3 sm:mt-0 sm:ml-0 sm:text-left w-full">
                                                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                                    Tinjau Masalah Pembayaran
                                                </h3>
                                                <div class="mt-4" x-show="activeIssue">
                                                    <p class="text-sm text-gray-500 font-bold">Pesan Pengemudi:</p>
                                                    <p class="text-sm text-gray-700 mb-4 bg-gray-50 p-2 rounded" x-text="activeIssue.message || 'Tidak ada pesan'"></p>
                                                    
                                                    <p class="text-sm text-gray-500 font-bold mb-2">File Bukti:</p>
                                                    ...
                                                    <template x-if="!activeIssue.file_path">
                                                        <p class="text-gray-400 italic">Tidak ada file diunggah</p>
                                                    </template>
                                                </div>
                                            </div>
                                        </div>
