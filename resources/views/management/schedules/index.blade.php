            ...
            >
                ...
                <div class="mb-6">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-2xl font-bold" x-text="canManageSchedules ? 'Kelola Jadwal' : 'Lihat Jadwal'"></h2>
                    </div>
                
                    <!-- Toolbar -->
                    <div class="w-full" x-data="{ showFilters: false }">
                        <div class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 mb-2">
                             <input type="text" x-model="filters.search" @keydown.enter="fetchData(1)" placeholder="Cari jadwal..." class="grow border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-2 h-[42px]">
                            ...
                            <!-- Actions Dropdown -->
                            <div class="relative" x-data="{ open: false }" @click.outside="open = false" x-show="canManageSchedules">
                            ...
                            <form action="{{ route('admin.schedules.create') }}" method="GET" class="ml-auto">
                                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700 text-center border border-transparent flex items-center justify-center h-[42px] whitespace-nowrap font-medium transition-colors">Tambah Jadwal</button>
                            </form>
                            @endcan
                        </div>
                        ...
                        <div x-show="showFilters" x-collapse x-cloak class="overflow-hidden">
                            <div class="w-full grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-md shadow-inner mb-6 mt-4 border border-gray-200">
                             <div>
                                ...
                                <select x-model="filters.sort_by" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-2">
                                    <option value="departure_time">Keberangkatan</option>
                                    <option value="arrival_time">Kedatangan</option>
                                    <option value="price_per_seat">Harga</option>
                                    <option value="remarks">Catatan</option>
                                </select>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select x-model="filters.filter_category" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 py-2">
                                    ...
                                    <option value="incomplete">Perlu Perhatian</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="overflow-x-auto relative min-h-[200px]">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            ...
                            <tr>
                            <tr>
                                <th class="px-6 py-3 text-left w-10" x-show="canManageSchedules">
                                    <input type="checkbox" 
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <template x-for="schedule in items" :key="schedule.id">
                                <tr :class="{'bg-blue-50': selectedItems.includes(schedule.id)}">
                                    <td class="px-6 py-4 whitespace-nowrap" x-show="canManageSchedules">
                                        <input type="checkbox" 
                                               class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                               :value="schedule.id"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="schedule.driver ? (schedule.driver.first_name + ' ' + (schedule.driver.last_name || '')) : 'Belum Ditugaskan'"></td>
                            ...
                            <tr x-show="items.length === 0 && !loading">
                                <td colspan="9" class="px-6 py-4 text-center text-gray-500">Tidak ada jadwal ditemukan.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
