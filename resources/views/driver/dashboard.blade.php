...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div class="block bg-indigo-600 rounded-lg shadow p-6 text-white hover:bg-indigo-700 transition">
            ...
            <a href="{{ route('driver.expenses') }}" class="block bg-indigo-600 rounded-lg shadow p-6 text-white hover:bg-indigo-700 transition">
            ...
            <a href="{{ route('driver.earnings') }}" class="block bg-indigo-600 rounded-lg shadow p-6 text-white hover:bg-indigo-700 transition">
                ...
                @else
                    <div class="grid gap-4">
                        ...
                        <div class="border rounded-lg p-4 flex justify-between items-center hover:bg-gray-50 transition">
                            <div>
                                <div class="font-bold text-lg">{{ $schedule->route->source_city }} <span class="text-gray-400">→</span> {{ $schedule->route->destination_city }}</div>
                                <div class="text-gray-600">Bus: {{ $schedule->bus->bus_number }} - {{ $schedule->bus->bus_name }}</div>
                                <div class="text-sm text-gray-500 mt-1">Departs: {{ \Carbon\Carbon::parse($schedule->departure_time)->format('H:i') }}</div>
                            </div>
                            <a href="{{ route('driver.schedules.show', $schedule->id) }}" class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">View & Check-in</a>
                        </div>
                        @endforeach
                    </div>
                @endif
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-900">
                                    {{ $schedule->route->source_city }} → {{ $schedule->route->destination_city }}
                                </td>
                                <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-600">
                                    {{ $schedule->bus->bus_number }}
                                </td>
                                <td class="px-3 py-2 whitespace-nowrap text-sm">
                                    <a href="{{ route('driver.schedules.show', $schedule->id) }}" class="text-indigo-600 hover:text-indigo-900">Details</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
